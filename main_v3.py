...
def expand_polygon(polygon, margin_deg):
    """
    向外膨胀多边形（每条边向外法向偏移），polygon: [[lng,lat],...]
    使用边法向偏移而非中心缩放，对细长建筑更准确。
    """
    n = len(polygon)
    # 计算每条边的外法向量
    edge_normals = []
    ...
    for i in range(n):
        ...
        if length < 1e-10:
            ...
            result.append([
                polygon[i][0] + nx/length * margin_deg,
                polygon[i][1] + ny/length * margin_deg
            ])
    return result
...
def line_intersects_any(p1, p2, polygons):
    """检查线段是否与任意多边形列表中的多边形相交"""
...
def get_bypass_waypoints(start_lng, start_lat, end_lng, end_lat,
                          polygon, safe_deg, strategy, all_expanded=None):
    """
    可视图（Visibility Graph）绕行算法：紧贴障碍物边缘绕行。
    思路：
      1. 膨胀多边形作为真正的禁飞区边界
      2. 确保多边形顶时针方向一致
      3. 从起点出发，沿多边形顶点序列（顺时针=右绕，逆时针=左绕）走到终点可见为止
      4. 结果是紧贴建筑轮廓的折线路径
    """
    ...
    # 统一为顺时针（方便右绕=顺序遍历，左绕=逆序遍历）
    area = polygon_signed_area(exp)
    if area > 0:  # 逆时针 -> 翻转为顺时针
        exp = exp[::-1]

    # 确定绕行方向：右绕=顺时针顺序，左绕=逆时针顺序
    # "向右绕行"：沿顺时针方向绕（exp本身已是顺时针）
    # "向左绕行"：沿逆时针方向绕（exp翻转）
    flight_dx = end_lng - start_lng
    flight_dy = end_lat - start_lat

    def choose_direction(exp_poly):
        """
        判断从起点看，多边形在飞行方向的左侧还是右侧。
        叉积 > 0 表示障碍物中心在飞行方向左侧。
        """
        cx = sum(p[0] for p in exp_poly) / len(exp_poly)
        cy = sum(p[1] for p in exp_poly) / len(exp_poly)
        cross = flight_dx * (cy - start_lat) - flight_dy * (cx - start_lng)
        return cross  # >0 障碍在左，<0 障碍在右

    obs_side = choose_direction(exp)

    if strategy == "向右绕行":
        # 无论障碍在哪侧，强制向右绕（顺时针顺序走多边形）
        walk_poly = exp[:]
    elif strategy == "向左绕行":
        # 强制向左绕（逆时针顺序走多边形）
        walk_poly = exp[::-1]
    else:
        # 最佳航线：障碍在左就右绕（绕右侧更近），障碍在右就左绕
        if obs_side > 0:
            walk_poly = exp[:]        # 障碍在左 -> 顺时针=右绕（走障碍右侧）
        else:
            walk_poly = exp[::-1]     # 障碍在右 -> 逆时针=左绕（走障碍左侧）

    m = len(walk_poly)

    # 找入口顶点：从起点出发，找第一个"能直视且最靠近起点方向"的顶点
    # 实际做法：找与起点->终点连线最先相交的边，取其端点
    def find_entry_vertex(src_lng, src_lat):
        """找从src出发沿walk_poly顺序第一个需要经过的顶点索引"""
        best_idx = -1
        best_proj = float('inf')
        for i, p in enumerate(walk_poly):
            # 顶点在飞行方向上的投影
            proj = (p[0]-src_lng)*flight_dx + (p[1]-src_lat)*flight_dy
            if proj > -safe_deg * 2:  # 在前方或附近
                if proj < best_proj:
                    best_proj = proj
                    best_idx = i
        return best_idx if best_idx >= 0 else 0

    # 构建绕行路径：从入口顶点沿walk_poly顺序走，直到能直视终点
    blocked = [exp]  # 用膨胀后的多边形做碰撞检测
    if all_expanded:
        blocked = all_expanded

    # 找入口：当前路径与哪个顶点相交处开始
    entry_idx = 0
    # 从起点到每个顶点，找第一个起点看不到（被遮挡）或在障碍物边界上的顶点
    # 更简单：直接遍历多边形顶点，找起点-终点连线第一次与多边形相交的边
    for i in range(m):
        p_a = walk_poly[i]
        p_b = walk_poly[(i+1) % m]
        if segments_intersect([start_lng, start_lat], [end_lng, end_lat], p_a, p_b):
            entry_idx = i
            break

    # 从 entry_idx 开始，沿 walk_poly 顺序收集顶点，直到从该顶点能直视终点
    result_lnglat = []
    visited = set()
    idx = entry_idx
    for _ in range(m * 2):  # 最多走2圈防死循环
        if idx in visited:
            break
        visited.add(idx)
        p = walk_poly[idx]
        result_lnglat.append(p)

        # 从这个顶点能直视终点且不穿越任何障碍物？
        can_see_end = not line_intersects_polygon(p, [end_lng, end_lat], exp)
        if can_see_end:
            break
        idx = (idx + 1) % m

    # 后处理：去掉可以跳过的中间顶点（可视图剪枝）
    if len(result_lnglat) > 2:
        pruned = [result_lnglat[0]]
        i = 0
        while i < len(result_lnglat) - 1:
            # 从当前点，找最远的可直视点
            j = len(result_lnglat) - 1
            while j > i + 1:
                if not line_intersects_polygon(pruned[-1], result_lnglat[j], exp):
                    break
                j -= 1
            pruned.append(result_lnglat[j])
            i = j
        result_lnglat = pruned

    return [[p[1], p[0]] for p in result_lnglat]  # [[lat, lng], ...]
...
def plan_route(strategy):
    ...
    # 预先膨胀所有需要绕行的障碍物多边形
    for obs in obs_data:
        obs["expanded"] = expand_polygon(obs["polygon"], safe_deg)
    ...
    while iteration < MAX_ITER:
        ...
        # 找最近的未绕过且需要绕行的障碍物（用膨胀多边形检测）
        hit = None
        hit_dist = float("inf")
        for obs in obs_data:
            if not obs["need_bypass"]:
                continue
            if line_intersects_polygon(p1, p2, obs["expanded"]):
                cx = sum(v[0] for v in obs["expanded"]) / len(obs["expanded"])
                cy = sum(v[1] for v in obs["expanded"]) / len(obs["expanded"])
